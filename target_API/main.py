...
def create_test_users():
    db = SessionLocal()
    # 2. Tạo User chuyên để test bị khóa (Cho Case ID 9)
    blocked_user = db.query(User).filter(User.username == "test_blocked").first()
    if not blocked_user:
        new_user = User(
            Id=999, # Một ID lạ không trùng ai
            username="test_blocked",
            password="password123",
            Ten="User Bi Khoa",
            Tuoi="20",
            role="user",
            is_active=False # <--- QUAN TRỌNG: Khóa luôn từ đầu
        )
        db.add(new_user)
        db.commit()
        print("✔ User test_blocked (is_active=False) da duoc tao")
    db.close()
@app.get("/")
def read_root():
